...
class LiveVisitors {
	...
	function add_meta_boxes() {
		...
		add_meta_box( 'social-proof-live', __( 'Realtime Visitors (Google&trade; Analytics)', 'wprtsp' ), array( $this, 'meta_box' ), 'socialproof', 'normal' );
	}
		...
		<table id="tbl_livestats" class="wprtsp_tbl wprtsp_tbl_livestats">
			<thead>
				...
				<tr>
					...
					<td><input id="wprtsp_livestats_title_notification" name="wprtsp[livestats_title_notification]" type="checkbox" value="1" <?php checked( 1, $livestats_title_notification, true ); ?>/></td>
				</tr>
				<tr>
...
function wprtsplive() {
	return LiveVisitors::get_instance();
}

wprtsplive();

class SalesMilestones {
	...
	function add_meta_boxes() {
		...
		add_meta_box( 'social-proof-hot-stats', __( 'Sales Milestones', 'wprtsp' ), array( $this, 'meta_box' ), 'socialproof', 'normal' );
	}
	...
	function meta_box() {
		...
		<table id="tbl_hotstats" class="wprtsp_tbl wprtsp_tbl_hotstats">
			...
			<tr>
				...
				<td><input id="wprtsp_hotstats_title_notification" name="wprtsp[hotstats_title_notification]" type="checkbox" value="1" <?php checked( 1, $hotstats_title_notification, true ); ?>/></td>
			</tr>
			<tr>
				...
				<td><input id="wprtsp_hotstats_sound_notification" name="wprtsp[hotstats_sound_notification]" type="checkbox" value="1" <?php checked( 1, $hotstats_sound_notification, true ); ?>/></td>
			</tr>
			<tr>
	...
	function send_data_wooc( $hotstats = array(), $settings ) {
		...
		$args      = apply_filters( 'wprtsp_' . $settings['conversions_shop_type'] . '_salesmilestones_args', $args, $settings );
		$orders    = wc_get_orders( $args );
		$orders    = apply_filters( 'wprtsp_' . $settings['conversions_shop_type'] . '_salesmilestones_data', $orders, $settings );
		$count     = count( $orders );
	...
	function send_data_edd( $hotstats, $settings ) {
		...
		$payments  = apply_filters( 'wprtsp_' . $settings['conversions_shop_type'] . '_salesmilestones_data', $payments, $settings );
		$threshold = $settings['hotstats_threshold'];
...
function wprtsphot() {
	return SalesMilestones::get_instance();
}
...
class CustomCTA {
	...
	function defaults( $defaults = array() ) {
		...
		$defaults['ctas']                         = array(
			array(
				'message'     => 'Get 15% discount with coupon INSERTCOUPONHERE',
				'button_text' => 'Buy Now',
				'link'        => '',
			),
		);
	...
	function meta_box() {
		...
		<thead>
			...
			<tr>
				...
				<td><input id="wprtsp_ctas_title_notification" name="wprtsp[ctas_title_notification]" type="checkbox" value="1" <?php checked( 1, $ctas_title_notification, true ); ?>/></td>
			</tr>
			<tr>
				...
				<td><input id="wprtsp_ctas_sound_notification" name="wprtsp[ctas_sound_notification]" type="checkbox" value="1" <?php checked( 1, $ctas_sound_notification, true ); ?>/></td>
			</tr>
			<tr>
		...
		<table id="ctas-fieldset-one" width="100%" class="wprtsp_tbl">
			...
			<tbody>
					...
					for ( $i = 0; $i < $count; $i++ ) {
						...
						?>
						<tr>
							<td><input type="text" class="widefat" name="wprtsp[ctas][<?php echo $i; ?>][message]" placeholder="Message" value="<?php echo htmlentities( $elem['message'] ); ?>" /></td>
							<td><input type="text" class="widefat" name="wprtsp[ctas][<?php echo $i; ?>][button_text]" placeholder="Button Text" value="<?php echo htmlentities( $elem['button_text'] ); ?>" /></td>
							<td><input type="url" class="widefat" name="wprtsp[ctas][<?php echo $i; ?>][link]" placeholder="Copy / Paste complete URL here" value="<?php echo htmlentities( $elem['link'] ); ?>" /></td>
					...
					<tr class="empty-row screen-reader-text">
						<td><input type="text" id="ctas_message_empty" placeholder="Message" class="widefat" name="wprtsp[ctas][<?php echo $i; ?>][message]" /></td>
						<td><input type="text" id="ctas_button_text_empty" placeholder="Button Text" class="widefat" name="wprtsp[ctas][<?php echo $i; ?>][button_text]" /></td>
						<td><input type="text" id="ctas_link_empty" placeholder="Copy / Paste complete URL here" class="widefat" name="wprtsp[ctas][<?php echo $i; ?>][link]" /></td>
					<td><a class="button remove-row" href="#">Remove</a></td>
				</tr>
			</tbody>
		</table>
